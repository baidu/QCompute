...
from Extensions.QuantumErrorProcessing.qcompute_qep.tomography import (
    ...
    MeasurementBasis,
    PreparationBasis,
    init_measurement_basis,
    init_preparation_basis,
)
...
from Extensions.QuantumErrorProcessing.qcompute_qep.quantum.channel import (
    QuantumChannel,
    PTM,
    unitary_to_ptm,
    ptm_to_process,
    ...
    def fit(self, qp: QProgram = None, qc: QComputer = None, **kwargs) -> QuantumChannel:
        ...
        **Examples**
        ...
        if len(self._qubits) != number_of_qubits(qp):
            raise ArgumentError(
                "ProcessTomography.fit(): the number of qubits in '@qp' "
        ...
        st = StateTomography(
            basis=self._meas_basis, method=self._method, shots=self._shots, ptm=True, qubits=self._qubits
        )
        ...
        if self._method.lower() == "inverse":  # The naive inverse method
            ...
            process_ptm = (
                la.pinv(dagger(M) @ M) @ dagger(M) @ ptm @ np.conjugate(P) @ la.pinv(np.transpose(P) @ np.conjugate(P))
            )
