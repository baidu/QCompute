    ...
    **Examples**
    ...
    if len(reg_sys) > 1:  # if we haven't prepared the target state
        if float_norm_half_1 != 0.0:  # if the first half needs to prepare
            ...
            circ_state_pre(reg_sys[1:], reg_ctrlling + [reg_sys[0]], list_float_state_half_1, reg_borrowed=reg_borrowed)
            X(reg_sys[0])
        if float_norm_half_2 != 0.0:  # if the second half needs to prepare
            ...
            circ_state_pre(reg_sys[1:], reg_ctrlling + [reg_sys[0]], list_float_state_half_2, reg_borrowed=reg_borrowed)


def circ_state_pre_inverse(
    reg_sys: List[QRegStorage],
    reg_ctrlling: Optional[List[QRegStorage]],
    list_float_target_state: List[float],
    reg_borrowed: Optional[List[QRegStorage]] = None,
    ...
    if len(reg_sys) > 1:
        if float_norm_half_2 != 0.0:
            ...
            circ_state_pre_inverse(
                reg_sys[1:], reg_ctrlling + [reg_sys[0]], list_float_state_half_2, reg_borrowed=reg_borrowed
            )
        if float_norm_half_1 != 0.0:
            ...
            X(reg_sys[0])
            circ_state_pre_inverse(
                reg_sys[1:], reg_ctrlling + [reg_sys[0]], list_float_state_half_1, reg_borrowed=reg_borrowed
            )
            X(reg_sys[0])
