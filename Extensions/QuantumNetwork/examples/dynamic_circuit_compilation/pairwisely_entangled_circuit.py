...
r"""
An example of pairwisely entangled circuit.
"""
...
cir = Circuit("Pairwisely entangled circuit")
...
for i in range(layer_num):
    ...
    for j in range(qubit_num):
        cir.h(j)
    # Entanglement layer
    for j in range(0, qubit_num - 1, 2):
        cir.cx([j, j + 1])
    for j in range(1, qubit_num - 1, 2):
        cir.cx([j, j + 1])
...
for i in range(0, qubit_num):
    for j in range(0, qubit_num):
        if abs(i - j) <= 2 * layer_num - 1:
            b_theory[i][j] += 1
for i in range(0, qubit_num - (2 * layer_num + 1) + 1, 2):
    b_theory[i][i + 2 * layer_num] += 1
for i in range(1, qubit_num - (2 * layer_num + 1) + 1, 2):
    b_theory[i + 2 * layer_num][i] += 1
...
for i in range(0, qubit_num):
    for j in range(0, qubit_num):
        if i - j >= 2 * layer_num + 1:
            c_theory[i][j] += 1
for i in range(1, qubit_num - (2 * layer_num + 1) + 1, 2):
    c_theory[i + 2 * layer_num][i] += 1
for i in range(0, qubit_num):
    for j in range(0, qubit_num):
        if j - i >= 2 * layer_num + 1:
            c_theory[i][j] += 1
for i in range(0, qubit_num - (2 * layer_num + 1) + 1, 2):
    c_theory[i][i + 2 * layer_num] += 1
...
for i in range(0, qubit_num - 2 * layer_num - 1):
    op_matrix[i][i + 2 * layer_num + 1] += 1
