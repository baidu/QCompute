...
def main():
    ...
    for bool_00 in [True, False]:
        for bool_01 in [True, False]: 
            for bool_10 in [True, False]:
                for bool_11 in [True, False]:
                    bool_0 = [bool_00, bool_01]
                    bool_1 = [bool_10, bool_11]
                    ...
                    shots = 1000
                    res = env.commit(shots=shots, fetchMeasure=True)

                    # Real value from mathematical calculation
                    realValue = test_real_value(
                        noises=noise_list, bool_gate_dict=bool_dict, shots=shots)

                    assert abs(res['counts']['00'] - realValue['00']) < 30
                    assert abs(res['counts']['01'] - realValue['01']) < 30
                    assert abs(res['counts']['10'] - realValue['10']) < 30
                    assert abs(res['counts']['11'] - realValue['11']) < 30
