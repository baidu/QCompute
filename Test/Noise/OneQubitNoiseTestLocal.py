...
def main():
    ...
    noise_list_total = [bfobj_01, adobj_005, cnobj, bpfobj_01, dpobj_1_01,
                  pnobj_005_01_015, pdobj_01, pfobj_01, rnobj_01_01]

    # Initialize a test circuit with noise
    noise_list = []
    for idex in range(len(noise_list_total)):
        noise_list.append(noise_list_total[idex])
        ...
        for bool_1 in [True, False]:
            for bool_2 in [True, False]: 
                ...
                if '0' in res['counts'].keys():
                    # print(res['counts']['0'], realValue['0'])
                    assert abs(res['counts']['0'] - realValue['0']) < 50
                else:
                    assert abs(res['counts']['1'] - realValue['1']) < 50
